...
export const badRequestHandler = (err, req, res, next) => {
    ...
    if (err.status === 400 || err.name === "ValidationError") {
      // Invia una risposta JSON con status 400
        ...
        next(err); // next passa al prossimo middleware se l'error non è 400.
    }
  };
...
export const genericErrorHandler = (err, req, res, next) => {
    // Logga lo stack trace dell'errore per debug ,err nei parametri funzione. 
    ...
    // Non c'è chiamata a next() perché questo è l'ultimo handler
  };
